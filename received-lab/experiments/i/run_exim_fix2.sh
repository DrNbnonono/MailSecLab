...
keepup; sleep 2
# pristine conf with SAFE permissions (this was the exim 'crash': docker cp kept 0777)
docker cp exim/exim.conf exim:/etc/exim/exim.conf
docker exec exim chmod 644 /etc/exim/exim.conf
docker restart exim >/dev/null; sleep 3
docker exec exim exim -bV >/dev/null 2>&1 && log "exim config OK" || { log "exim still broken"; exit 1; }
retarget_exim() {
