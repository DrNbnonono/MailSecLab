...
mkpayload() { # CASE SEP
    docker exec mail-client python3 /scripts/mk_i1_payload.py "$1" "$2"
}
log "== I1 v3: per-pair payload, self-healing =="
keepup; sleep 2
retarget_mta1() {
    case $1 in
...
for pair in "postfix1 postfix2" "postfix1 postfix2n" "postfix1 exim" "postfix1 opensmtpd" "postfix1 mailpit" \
    ...
    for sep in crlf lf; do
        keepup; sleep 1
        c="I1-$m1-to-$m2-$sep"
        mkpayload $c $sep >/dev/null
        r=$($MC /scripts/send_bdat.py $m1 25 /results/i-series/$c.payload $c 2>&1 | grep -E 'CHUNKING|BDAT_REPLY' | tr '\n' ' ')
        sleep 3
        keepup; sleep 1
